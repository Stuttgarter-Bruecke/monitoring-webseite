...
function(d3, queue, $) {
  ...
  function makeGraphs(error, dataContext, data) {
    ...
    //
    y.domain([
        d3.min(samples, function(c) { return d3.min(c.values, function(v) { return v.temperature; }); }),
        d3.max(samples, function(c) { return d3.max(c.values, function(v) { return v.temperature; }); })
    ]);
  ...
  function brushed() {
    ...
    //  selectAll("area")
    //secondBrush
    //  //.attr("d", function(d) { return markedArea(d.values); })
    //  .attr("x", x(range[0]))
    //  .attr("y", 0)
